/* LES 400 ETIQUETTES DU MODELE, DANS L'ORDRE DE SES SORTIES.
 *
 * COPIEES TELLES QUELLES depuis la demonstration officielle dont vient le
 * modele : MTG/essentia.js, examples/demos/discogs-autotagging,
 * views/src/audio/labels.js. Le model.json et les onze fichiers de poids
 * servis par notre passerelle sont ceux de cette demonstration, au meme
 * octet pres (64 422 octets pour le descripteur).
 *
 * ═══ POURQUOI CE FICHIER EXISTE, ET POURQUOI IL NE SE REECRIT PAS A LA MAIN ═══
 *
 * Jusqu'au 22 septembre 2026, les sorties du reseau etaient nommees avec
 * l'ordre des classes du fichier de metadonnees d'UN AUTRE MODELE d'Essentia,
 * la tete discogs400 pour EffnetDiscogs, qui range « Brass & Military »
 * avant « Children's ». Cette demonstration-ci range les memes 400 noms dans
 * un autre ordre, et ce n'est meme pas le meme ensemble : six noms different.
 * A 268 index sur 400, le nom affiche etait celui d'une autre classe. Mesure
 * sur le banc le 22 septembre 2026 : la sortie que la page appelait
 * « Hi NRG » (index 88) etait « House », « Dance-pop » (47) etait « Deep
 * House », « Hardstyle » (87) etait « Hip-House », « Tech House » (127)
 * etait « Techno », « Power Electronics » (115) etait « Progressive House ».
 * Le trio « Hi NRG / Dance-pop / Hardstyle » que Mika voyait sur de la deep
 * house etait donc « House / Deep House / Hip-House » : le reseau avait
 * raison, la page le nommait mal. Ni le micro ni le reseau n'etaient en
 * cause, et c'est le banc (scripts/banc-reconnaitre.mjs) qui l'a montre.
 *
 * Un test (etiquettes-modele.test.ts) verifie que cet ordre et la table de
 * correspondance portent le meme ensemble de 400 noms : une etiquette du
 * modele sans entree dans la table serait un style qu'on ne saurait plus
 * ramener a l'atlas. */

export const ETIQUETTES_MODELE: readonly string[] = [
  "Blues---Boogie Woogie",
  "Blues---Chicago Blues",
  "Blues---Country Blues",
  "Blues---Delta Blues",
