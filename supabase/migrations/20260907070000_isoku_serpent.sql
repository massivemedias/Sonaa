-- ISOKU : le jeu Serpent remplace Blocs. Appliquee le 7 septembre 2026.
...
alter table public.isoku_scores add constraint isoku_scores_mode_check
  check (mode in ('sudoku','tours','cube','mahjong','deux','picross','mines','blocs','chute','trio','tetris','serpent'));
...
returns integer language sql immutable as $$
  select round(
    ...
    * (case mode when 'tours' then 0.6 when 'cube' then 0.4 when 'mahjong' then 0.5 when 'deux' then 0.5 when 'picross' then 0.8 when 'mines' then 0.7 when 'blocs' then 0.5 when 'chute' then 0.5 when 'trio' then 0.6 when 'tetris' then 0.6 when 'serpent' then 0.5 else 1.0 end)
    * least(2.0, greatest(0.5,
        (case mode
           ...
           when 'trio'    then (case difficulte when 'facile' then 180 when 'moyen' then 300 when 'difficile' then 420 else 600 end)
           when 'tetris'  then (case difficulte when 'facile' then 240 when 'moyen' then 480 when 'difficile' then 720 else 1200 end)
           when 'serpent' then (case difficulte when 'facile' then 120 when 'moyen' then 240 when 'difficile' then 360 else 600 end)
