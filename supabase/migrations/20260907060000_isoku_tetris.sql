-- ISOKU : le jeu Tetris. Appliquee le 7 septembre 2026.
...
alter table public.isoku_scores add constraint isoku_scores_mode_check
  check (mode in ('sudoku','tours','cube','mahjong','deux','picross','mines','blocs','chute','trio','tetris'));
...
returns integer language sql immutable as $$
  select round(
    ...
    * (case mode when 'tours' then 0.6 when 'cube' then 0.4 when 'mahjong' then 0.5 when 'deux' then 0.5 when 'picross' then 0.8 when 'mines' then 0.7 when 'blocs' then 0.5 when 'chute' then 0.5 when 'trio' then 0.6 when 'tetris' then 0.6 else 1.0 end)
